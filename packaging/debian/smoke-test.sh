...
for _ in $(seq 1 20); do
  if python3 -c "import http.client; c=http.client.HTTPConnection(\"127.0.0.1\",19140,timeout=1); c.request(\"GET\",\"/health\"); r=c.getresponse(); b=r.read().decode(); assert r.status in (200,503) and \"1.18.3\" in b"; then ready=1; break; fi
  sleep 1
done
test "$ready" = 1
