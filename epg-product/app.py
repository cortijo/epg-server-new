...
function showMainPage(page){mainPage=page==='carriers'?'carriers':'monitor';document.querySelector('.toolbar h2').textContent=mainPage==='monitor'?'Monitoramento de canais':'Gestão de portadoras';document.querySelector('.toolbar .muted').textContent=mainPage==='monitor'?'Situação da emissão multicast e grade EPG carregada.':'Visualize por modulador ou gerencie os canais diretamente.';el('railMonitor')?.classList.toggle('active',mainPage==='monitor');el('railCarriers')?.classList.toggle('active',mainPage==='carriers');document.querySelector('.panel-filter').style.display=mainPage==='monitor'?'none':'flex';render()}
