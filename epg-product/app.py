...
document.body.insertAdjacentHTML('afterbegin','<aside class="app-rail"><div class="rail-logo">E</div><nav class="rail-nav"><button class="active" title="Portadoras">▶</button><button onclick="openTimeline()" title="Grade de programação">▦</button><button onclick="openSources()" title="Fontes XMLTV">⇄</button><button onclick="openPublications()" title="Publicações XMLTV">≡</button><button onclick="openEpgErrors()" title="Erros do EPG">!</button><button onclick="openAbout()" title="Sobre">⚙</button></nav><button class="rail-bottom" onclick="location.reload()" title="Atualizar painel">↻</button></aside>');
let state={carriers:[],sources:[]},sources=[],catalog=[],session={user:null},users=[],publications=[],expandedCarriers=new Set(),guideCache={},timelineData=null,timelineStart=0,timelineWindow=8*3600,timelineDay=0;const el=id=>document.getElementById(id),esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
document.querySelector('.summary')?.insertAdjacentHTML('afterend','<div class="panel-filter"><input id="carrierFilter" placeholder="Filtrar por portadora, multicast, TSID ou ONID" oninput="filterCarriers()"><button class="filter-pill active" onclick="setCarrierFilter(this,\'all\')">Todas</button><button class="filter-pill" onclick="setCarrierFilter(this,\'running\')">Em transmissão</button><button class="filter-pill" onclick="setCarrierFilter(this,\'error\')">Com erro</button></div>');
...
function filterCarriers(){const term=(el('carrierFilter')?.value||'').toLocaleLowerCase('pt-BR');document.querySelectorAll('.carrier-table tr.main-row').forEach(row=>{const status=carrierStatusFilter==='all'||(carrierStatusFilter==='running'&&row.querySelector('.badge.running'))||(carrierStatusFilter==='error'&&(row.classList.contains('health-error')||row.querySelector('.badge.error'))),show=status&&(!term||row.textContent.toLocaleLowerCase('pt-BR').includes(term));row.hidden=!show;const details=row.nextElementSibling;if(details?.classList.contains('program-row'))details.hidden=!show})}
