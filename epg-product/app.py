...
let carrierStatusFilter='all',carrierSortMode='name',carrierSortDirection=1;
...
function sortedCarriers(carriers){return [...carriers].sort((a,b)=>carrierSortDirection*(carrierSortMode==='multicast'?multicastSortKey(a)-multicastSortKey(b)||String(a.name||'').localeCompare(String(b.name||''),'pt-BR',{numeric:true}):String(a.name||'').localeCompare(String(b.name||''),'pt-BR',{numeric:true,sensitivity:'base'})))}
function setCarrierSort(mode){const next=mode==='multicast'?'multicast':'name';carrierSortDirection=carrierSortMode===next?-carrierSortDirection:1;carrierSortMode=next;render();filterCarriers()}
...
async function ensureCatalogFor(select){const row=select.closest('.service-edit'),list=row?.querySelector('datalist'),input=row?.querySelector('.s-channel'),sourceId=select.value||el('cSource')?.value;if(!list||!sourceId)return;const previous=list.dataset.sourceId,requestId=String(++catalogRequestSequence);list.dataset.sourceId=sourceId;list.dataset.requestId=requestId;list.innerHTML='';if(previous!==undefined&&previous!==sourceId&&input)input.value='';if(input)input.placeholder='Carregando canais da fonte…';try{const channels=(await api(`/api/catalog?source_id=${encodeURIComponent(sourceId)}`)).channels||[];if(list.dataset.requestId!==requestId||(select.value||el('cSource')?.value)!==sourceId)return;list.innerHTML=channels.map(c=>`<option value="${esc(c.id)}">${esc(c.name)}</option>`).join('');if(input)input.placeholder=channels.length?'Digite ou selecione um canal':'Nenhum canal disponível nesta fonte'}catch(e){if(list.dataset.requestId!==requestId)return;list.innerHTML='';if(input)input.placeholder='Falha ao carregar canais';toast(e.message,true)}}
document.addEventListener('change',event=>{if(event.target?.id==='cSource')document.querySelectorAll('.service-edit .s-source').forEach(select=>{if(!select.value)ensureCatalogFor(select)})});
...
async function populateChannelCatalog(sourceId){const list=el('channelDirectCatalog'),input=el('channelXmltv');if(!list||!sourceId)return;const previous=list.dataset.sourceId,requestId=String(++catalogRequestSequence);list.dataset.sourceId=sourceId;list.dataset.requestId=requestId;list.innerHTML='';if(previous!==undefined&&previous!==sourceId&&input)input.value='';if(input)input.placeholder='Carregando canais da fonte…';try{const channels=(await api(`/api/catalog?source_id=${encodeURIComponent(sourceId)}`)).channels||[];if(list.dataset.requestId!==requestId)return;list.innerHTML=channels.map(item=>`<option value="${esc(item.id)}">${esc(item.name)}</option>`).join('');if(input)input.placeholder=channels.length?'Digite ou selecione um canal':'Nenhum canal disponível nesta fonte'}catch(_){if(list.dataset.requestId!==requestId)return;list.innerHTML='';if(input)input.placeholder='Falha ao carregar canais'}}
...
setInterval(()=>{if(mainPage==='monitor'&&monitorGuideExpanded)loadMonitorGuide()},60000);
showMainPage('monitor');
document.head.insertAdjacentHTML('beforeend','<style>.table-sort{display:flex;align-items:center;justify-content:space-between;width:100%;padding:0;border:0;background:transparent;color:inherit;font:inherit;text-transform:inherit;letter-spacing:inherit;text-align:left}.table-sort:hover{color:var(--blue);background:transparent}.table-sort i{font-style:normal;font-size:13px;color:#7c8da1}.table-sort.active i{color:var(--blue)}</style>');
let monitorGuideExpanded=false;
document.head.insertAdjacentHTML('beforeend','<style>.channel-monitor-grid{display:grid;grid-template-columns:repeat(3,minmax(270px,1fr));gap:13px;margin-bottom:14px}.channel-monitor-card{position:relative;background:#fff;border:1px solid var(--line);border-top:4px solid #8797aa;border-radius:10px;padding:14px;box-shadow:0 2px 8px #17395c0d;min-height:174px}.channel-monitor-card.ok{border-top-color:var(--green)}.channel-monitor-card.warning{border-top-color:var(--amber)}.channel-monitor-card.error{border-top-color:var(--red)}.channel-monitor-head{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}.channel-monitor-head h3{font-size:14px;margin:0;color:#12314e}.channel-monitor-meta{font-size:11px;color:var(--muted);margin-top:4px}.channel-monitor-now{margin-top:15px;padding-top:12px;border-top:1px solid var(--line)}.channel-monitor-now small,.channel-monitor-next small{display:block;font-size:9px;font-weight:900;color:var(--muted);letter-spacing:.06em;margin-bottom:3px}.channel-monitor-now b{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.channel-monitor-next{margin-top:11px;font-size:12px}.channel-monitor-message{margin-top:11px;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.monitor-errors-wide{margin-bottom:14px}.monitor-guide-toggle{width:100%;display:flex;align-items:center;justify-content:space-between;padding:16px 18px;background:#fff;border:1px solid var(--line);border-radius:10px;color:#12314e;font-weight:800}.monitor-guide-toggle:hover{border-color:var(--blue);background:#f7fbff}.monitor-guide-toggle i{font-style:normal;font-size:17px}.monitor-guide-toggle small{margin-left:8px;font-weight:400}.monitor-guide-expanded{margin-top:10px}@media(max-width:1250px){.channel-monitor-grid{grid-template-columns:repeat(2,minmax(270px,1fr))}}@media(max-width:760px){.channel-monitor-grid{grid-template-columns:1fr}}</style>');
function monitorChannelCards(){const carriers=state.carriers||[];return (state.epg_health?.carriers||[]).flatMap(group=>group.services.map(service=>{const carrier=carriers.find(item=>item.id===service.carrier_id),configured=carrier?.services?.find(item=>item.id===service.service_id),label=service.status==='ok'?'Normal':service.status==='warning'?'Atenção':'Erro';return `<article class="channel-monitor-card ${service.status}"><div class="channel-monitor-head"><div><h3><i class="health-dot ${service.status}"></i>${esc(service.service_name)}</h3><div class="channel-monitor-meta">${esc(group.name)} · SID ${esc(configured?.service_id||'—')}</div></div><span class="badge ${service.status==='error'?'error':service.status==='ok'?'running':''}">${label}</span></div><div class="channel-monitor-now"><small>NO AR AGORA</small><b>${esc(service.current_title||'Sem programa no ar')}</b></div><div class="channel-monitor-next"><small>A SEGUIR</small>${esc(service.next_title||'Não informado')}</div><div class="channel-monitor-message health-message ${service.status}" title="${esc(service.message)}">${esc(service.message)}</div></article>`})).join('')}
function toggleMonitorGuide(){monitorGuideExpanded=!monitorGuideExpanded;render();if(monitorGuideExpanded&&!monitorGuide)loadMonitorGuide()}
renderMonitor=function(){const carriers=state.carriers||[],health=state.epg_health||{},services=carriers.flatMap(c=>c.services),issues=health.errors||[],epgIssues=issues.filter(e=>e.code!=='emitter_stopped'),activeServices=carriers.filter(c=>c.active).reduce((n,c)=>n+c.services.length,0),guideCount=monitorGuide?`${monitorGuide.services.filter(monitorMatches).length} canais · ${monitorGuide.services.filter(monitorMatches).reduce((total,item)=>total+(item.schedule||[]).length,0)} programas exibidos`:'A programação será carregada ao abrir';el('mCarriers').textContent=carriers.length;el('mActive').textContent=carriers.filter(c=>c.active).length;el('mServices').textContent=services.length;el('mRestarts').textContent=carriers.reduce((n,c)=>n+(c.restart_count||0),0);el('carrierTable').innerHTML=`<section class="monitor-summary"><div class="monitor-kpi"><small>CANAIS CADASTRADOS</small><b>${services.length}</b></div><div class="monitor-kpi ok"><small>CANAIS EM EMISSÃO</small><b>${activeServices}</b></div><div class="monitor-kpi ${issues.length?'error':'ok'}"><small>ERROS GERAIS</small><b>${issues.length}</b></div><div class="monitor-kpi ${epgIssues.length?'warning':'ok'}"><small>ERROS DE EPG</small><b>${epgIssues.length}</b></div></section><section class="channel-monitor-grid">${monitorChannelCards()||'<div class="card empty"><h3>Nenhum canal cadastrado</h3></div>'}</section><div class="monitor-card monitor-errors-wide"><div class="monitor-card-head"><h3>Erros e alertas</h3><button onclick="openEpgErrors()">Ver central</button></div><div class="monitor-table-wrap"><table class="monitor-table"><tbody>${issues.slice(0,20).map(e=>`<tr><td><b>${esc(e.service_name)}</b><div class="muted">${esc(e.carrier_name)} · ${esc(e.source_name)}</div></td><td><span class="health-message ${e.status}">${esc(e.message)}</span></td></tr>`).join('')||'<tr><td><i class="health-dot ok"></i>Todos os canais estão normais.</td></tr>'}</tbody></table></div></div><button class="monitor-guide-toggle" onclick="toggleMonitorGuide()"><span>Grade EPG completa — hoje <small class="muted">${esc(guideCount)}</small></span><i>${monitorGuideExpanded?'▲':'▼'}</i></button>${monitorGuideExpanded?`<div class="monitor-card monitor-guide monitor-guide-expanded"><div class="monitor-card-head"><div><h3>Programação carregada</h3><small id="monitorProgrammeCount" class="muted">${esc(guideCount)}</small></div><div class="monitor-guide-actions"><input id="monitorGuideSearch" value="${esc(monitorSearch)}" placeholder="Pesquisar canal ou programação" oninput="filterMonitorGuide(this.value)"><button id="monitorRefreshButton" class="monitor-refresh-button" onclick="refreshMonitorGuide()"><span class="refresh-icon">⟳</span> Atualizar grade</button></div></div><div id="monitorGuideContent" class="monitor-guide-scroll">${renderMonitorGuide(monitorGuide)}</div></div>`:''}`;if(monitorGuideExpanded&&!monitorGuide)loadMonitorGuide()};
const renderWithCarrierOrder=render;
function applyCarrierSortHeaders(){if(overviewMode!=='carriers')return;const headers=document.querySelectorAll('.carrier-table thead th'),arrow=carrierSortDirection===1?'▲':'▼';if(headers.length<2)return;headers[0].innerHTML=`<button class="table-sort ${carrierSortMode==='name'?'active':''}" onclick="setCarrierSort('name')"><span>Portadora</span><i>${carrierSortMode==='name'?arrow:'↕'}</i></button>`;headers[1].innerHTML=`<button class="table-sort ${carrierSortMode==='multicast'?'active':''}" onclick="setCarrierSort('multicast')"><span>Destino multicast</span><i>${carrierSortMode==='multicast'?arrow:'↕'}</i></button>`}
function moveMonitorPanelsToTop(){if(mainPage!=='monitor')return;const container=el('carrierTable'),cards=container?.querySelector('.channel-monitor-grid'),errors=container?.querySelector('.monitor-errors-wide'),toggle=container?.querySelector('.monitor-guide-toggle'),guide=container?.querySelector('.monitor-guide-expanded');if(!cards)return;for(const panel of [errors,toggle,guide])if(panel)container.insertBefore(panel,cards)}
render=function(){state.carriers=sortedCarriers(state.carriers||[]);renderWithCarrierOrder();applyCarrierSortHeaders();moveMonitorPanelsToTop()};
