...
async function refresh(){try{const loaded=await Promise.all([api('/api/state'),api('/api/sources'),api('/api/session')]);state=loaded[0];sources=loaded[1].sources;session=loaded[2];el('usersButton').style.display=session.user?.role==='admin'?'':'none';el('timelineButton').disabled=!(state.carriers||[]).length;el('newCarrierButton').disabled=!state.license?.valid;render();el('clock').textContent=`${session.user?.display_name||''} · ${new Date().toLocaleTimeString('pt-BR')}`}catch(e){toast(e.message,true)}}
